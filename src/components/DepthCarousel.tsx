...
const DEFAULT_ITEMS: DepthCarouselItemObject[] = [
  { image: 'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?auto=format&fit=crop&w=800&q=85', alt: 'Laptop', title: 'AeroBook Pro 16', category: 'Computing' },
  { image: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=800&q=85', alt: 'Headphones', title: 'SonicPro Wireless', category: 'Studio Audio' },
  { image: 'https://images.unsplash.com/photo-1592750475338-74b7b21085ab?auto=format&fit=crop&w=800&q=85', alt: 'Smartphone', title: 'Horizon Titanium', category: 'Mobile' },
  { image: 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&w=800&q=85', alt: 'Smartwatch', title: 'Pulse Horizon GPS', category: 'Wearables' },
  { image: 'https://images.unsplash.com/photo-1600080972464-8e5f35f63d08?auto=format&fit=crop&w=800&q=85', alt: 'Gaming Gear', title: 'Apex Pro Controller', category: 'Gaming' }
];
...
export const DepthCarousel: React.FC<DepthCarouselProps> = ({
  ...
  const wheelTimerRef = useRef<NodeJS.Timeout | number | null>(null);
      ...
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            ...
            {/* Optional product label overlay only if showCardText is true */}
            {showCardText && (item.title || item.category || item.price) && (
              <div className="depth-carousel__info">
                {item.category && (
                  <span className="text-[10px] font-bold uppercase tracking-widest text-[#078F83]">
                    {item.category}
                  </span>
                )}
                {item.title && (
                  <span className="text-[15px] font-extrabold text-[#063D37] leading-tight drop-shadow-none truncate">
                    {item.title}
                  </span>
                )}
                {item.price && (
                  <span className="text-[13px] font-bold text-[#4A635E] mt-0.5">
                    {item.price}
                  </span>
                )}
              </div>
